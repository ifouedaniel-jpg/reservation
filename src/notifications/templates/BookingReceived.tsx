...
const s = {
  ...
  box: { backgroundColor: '#f9fafb', borderRadius: '8px', padding: '16px 20px', margin: '24px 0' },
...
export default function BookingReceived({ booking }: { booking: BookingSnapshot }) {
  ...
  return (
    <Html lang="fr">
      <Head />
      <Preview>Votre demande est en attente de validation</Preview>
      <Body style={s.body}>
        <Container style={s.container}>
          ...
          <Section style={s.content}>
            <Text style={s.h1}>Demande reçue</Text>
            <Text style={s.p}>Bonjour {booking.customerFirstName},</Text>
            <Text style={s.p}>
              Nous avons bien reçu votre demande pour{' '}
              <strong>{booking.service.name}</strong>. Elle est en attente de
              validation — vous serez contacté(e) très bientôt.
            </Text>
            ...
            <Section style={s.box}>
              ...
              <Text style={s.row}>
                ...
                <a href={trackingUrl} style={s.link}>{booking.publicCode}</a>
              </Text>
            </Section>

            <Text style={s.p}>
              Vous pouvez suivre votre réservation à tout moment :{' '}
